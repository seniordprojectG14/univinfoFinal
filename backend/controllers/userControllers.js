...
const DOMAIN = 'sandbox576e1a801e6347e2872a6192d9be6129.mailgun.org';
const ApiKey = '9b66cacfea26c37e76c6e9b07330286c-15b35dee-ee47bf84';
...
const registerUser = asyncHandler(async (req, res) => {
  const { name, username, password, email, pic } = req.body;
...
//@description     Auth the user, resetting password
//@route           POST /api/users/ResetPassword.js
//@access          Public
const ForgotPassword = asyncHandler(async (req, res) => {
  ...
  let TempPassword = genPassword(10);
  const salt = await bcrypt.genSalt(10);
  let hashedTempPassword = await bcrypt.hash(TempPassword, salt);

  if (user) {
  const updatedPost = await User.findByIdAndUpdate(user._id, { password: hashedTempPassword }, { new: true });
  res.json(updatedPost);
  } else {
    res.status(401);
    throw new Error("Invalid username or Password");
  ...
  //Sending mail
  //=====================================================
  let msg = `Hello user, you username is ${ user.username }, and password is ${ TempPassword  }.`

  const data = {
    from: 'me@samples.mailgun.org',
    to: email,
    subject: 'Password Recorvery',
    text: `Hello user, you username is ${ user.username }, and temporary password is ${ TempPassword}, please change your password once you login to you account`,
  };
...
const updateUser = asyncHandler(async (req, res) => {
  ...
  const {newNickName,
        email,
        password} = req.body;
  //console.log(JSON.stringify(Newusername) + " = newusername");

  const salt = await bcrypt.genSalt(10);
  let hashedPassword = await bcrypt.hash(password, salt);

  if (currentUser) {
    const updatedPost = await User.findByIdAndUpdate(currentUser._id, { password: hashedPassword}, { new: true });
    const updatedName = await User.findByIdAndUpdate(currentUser._id, { name: newNickName}, { new: true });
    const updatedEmail = await User.findByIdAndUpdate(currentUser._id, { email: email}, { new: true });
    res.json(updatedPost);
    } else {
      res.status(401);
    ...
    console.log(JSON.stringify(currentUser) + " = newusername");

  
  
});

 export {allUsers, registerUser, authUser, ForgotPassword,deleteUser,updateUser};
