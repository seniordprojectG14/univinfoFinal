...
const PostItem = ({ post, setCurrentId, user, setUser, proplist }) => {
  ...
  const dispatchBothActions = async (
    postId,
    count,
    username,
    postListDisLikeUsernames,
    post
  ) => {
    dispatch(
      addDisLike(postId, count, username, postListDisLikeUsernames, post)
    );
    //await wait(2000);
    dispatch(
      dislikePost(postId, count, username, postListDisLikeUsernames, post)
    );

    dispatch(
      subUsernameLikes(postId, count, username, postListDisLikeUsernames, post)
    );
  };

  const dispatchBothActionsLike = async (
    postId,
    count,
    username,
    postListDisLikeUsernames,
  ...
  const dispatchBothActionsMiss = async (
    postId,
    count,
    username,
    postListMissUsernames,
    post
  ) => {
        ...
        <div className={classes.actions}>
          {post?.postListLikeUsernames.includes(user?.username) && (
            <Button
              ...
              color="secondary"
              onClick={() => {
                dispatchBothActionsLike(
                  post?._id,
                  post?.likeCount,
                  user?.username,
                  post?.postListDisLikeUsernames,
                  post
                );
              }}
            >
              <ThumbUpAltIcon color="secondary" fontSize="small" /> Like{" "}
            </Button>
          )}
          ...
          {post?.postListDisLikeUsernames.includes(user?.username) && (
            ...
            >
              <ThumbDownIcon color="secondary" fontSize="small" /> disLike{" "}
            </Button>
          )}
          {usernameNotInList(
            post?.postListDisLikeUsernames,
          ...
          <Button
            onClick={() => {
              dispatchBothActionsMiss(
                post?._id,
                post?.MissCount,
                user?.username,
                post?.postListMissUsernames,
                post
              );
            }}
          >
